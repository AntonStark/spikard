...
/**
 * Узлов сочинения на самом деле не будет. Вместо них также будут узлы подчинения. Однако, в тех местах,
 * где возможно сразу несколько вариантнов, дерево будет расслаиваться.
 * То есть потомок может превращаться в (виртуальный) узел расслоения.
 *
 * И последнее, что нужно использовать, это информация о Типах. Найдя подходящее имя, мы знаем
 * Типы его аргументов if any и можем сузить ...
 ...
 * Общий Тип Кортеж и Тип Функция, тогда \cdot:Функция(\cdot:Кортеж)
 *
 * Возвращаясь к использованию информации о типах. Дело в том, что имена, допускающие связывание (может быть полезно
 * разделять на те, где связывание возможно и где обьязательно?), без него имеют один Тип, а после другой.
 * Например, Natural->Real становится просто Real. Что если считать, что имя со связыванием имеет тот Тип, который
 * будет после связывания? И, рассматривая скобки как одну из особых конструкций, можно перестать
 * думать, что имя f поменяло тип став f(x). Раз и навсегда f:Map(Real, Real) а х:Real
 *
 * Однако какой тогда Тип у \cdot:Функция(\cdot:Кортеж)? Он зависит от того какой Тип у функции. Привет, зависимые Типы!
 */
